...
func (h *Handler) buildTicketRequest(req *PlaceTicketRequest) *models.TicketRequest {
	...
	// Set default channel and language
		...
		// Get LimitID from config
		var limitID int64
		if h.cfg.LimitID != "" {
			var err error
			limitID, err = strconv.ParseInt(h.cfg.LimitID, 10, 64)
			if err != nil {
				log.Printf("Warning: Failed to parse LimitID from config '%s': %v, using 0.", h.cfg.LimitID, err)
				limitID = 0
			}
		}
