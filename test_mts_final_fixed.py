#!/usr/bin/env python3
"""
MTS Service API 最终测试脚本 (修复 Banker 投注)
"""
...
class MTSFinalTester:
    ...
    def test_banker_system_success_fixed(self):
        """TC-BANKER-001-FIXED: 成功提交 Banker 系统投注 (修复后)"""
        self.log("开始测试: 成功提交 Banker 系统投注 (修复后)", "TEST")
        data = {
            "ticketId": self.generate_ticket_id("banker-fixed"),
            "bets": [
                {
                    "type": "system",
                    "selectionRefs": [
                        {"selectionIndex": 0, "banker": True},
                        {"selectionIndex": 1},
                        {"selectionIndex": 2},
                        {"selectionIndex": 3}
                    ],
        ...
        self.record_result("TC-BANKER-001-FIXED", "成功提交 Banker 系统投注 (修复后)", result, "200 OK")

    def run_all_tests(self):
        """运行所有测试"""
        self.log("=" * 60, "INFO")
        self.log("开始 MTS API 最终测试 (修复 Banker)", "INFO")
        event_id_preview = ", ".join(self.event_ids[:5])
        self.log(f"使用的赛事 ID: {event_id_preview}...", "INFO")
        self.log("=" * 60, "INFO")

        self.test_banker_system_success_fixed()

        self.log("=" * 60, "INFO")
        self.log("测试完成", "INFO")
        self.log("=" * 60, "INFO")

    def generate_report(self, output_file: str = "final_test_results_fixed.json"):
        """生成测试报告"""
